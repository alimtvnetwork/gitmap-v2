...
export const changelog: ChangelogEntry[] = [
  {
    ...
    items: [
      ...
      "Added `ErrNppZipNotFound`, `ErrNppSourceDir`, `ErrNppDirCreate`, `ErrNppExtractEntry`, `ErrNppFileCreate`, `ErrNppFileCopy`, `ErrNppFileRead`, `ErrNppFileWrite`, `ErrNppWindowsOnly`, `ErrNppNoAppData` constants with exact path and operation context.",
      "Updated `ErrConfigLoad` to include config file path: `Error: failed to load config from %s: %v (operation: read)`.",
      "Updated `ErrScanFailed` to include scan directory: `Error: scan failed on directory %s: %v (operation: resolve)`.",
      "Updated `ErrCloneFailed` to include source file path: `Error: clone failed for source file %s: %v (operation: read)`.",
      "Updated `ErrCreateDir` and `ErrCreateFile` to include exact target path and operation.",
      "Updated `ErrMigrationFailed` to include directory path and reason: `Error: failed to migrate directory %s: %v (operation: move)`.",
      "Updated `ErrSSHKeygen`, `ErrSSHReadPub`, `ErrSSHConfig`, `ErrSSHFingerprint` to include key/config file paths.",
      "Updated `ErrAssetBuildFailed`, `ErrAssetUploadFinal`, `ErrAssetNoToken`, `ErrAssetRemoteParse` with operation context.",
      "Updated `ErrTaskLoadFile`, `ErrTaskSaveFile`, `ErrTaskSrcNotExist`, `ErrTaskDestCreate` to include file/directory paths.",
      "Updated `ErrInstallExeNotFound` with path and reason: `Error: post-install binary not found at %s (operation: verify)`.",
      "Eliminated all silent error swallowing in `installnpp.go` — all 8 previously swallowed errors now log exact path, operation, and reason.",
      "Added spec: `spec/02-app-issues/error-management-file-path-and-missing-file-code-red-rule.md`.",
      "Added NPP installation variants: `npp` (NPP + Settings), `npp-settings` (NPP Settings), `install-npp` (Install NPP).",
      "Added post-install binary verification via `verifyExePath` for NPP and VS Code on Windows.",
