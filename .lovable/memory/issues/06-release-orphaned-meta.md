...
When `gitmap release` detects a `.release/vX.Y.Z.json` file but neither
...
The `.release/` directory must **NEVER** be modified by the AI/editor.
Release metadata JSON files are local build artifacts managed exclusively
by the CLI tool. The AI must not create, edit, or delete any files in `.release/`.
